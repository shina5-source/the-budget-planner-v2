...
export function useBudgetPeriod() {
   ...
   * La période va du jour de paie du mois actuel au jour avant la paie du mois suivant
   * 
   * CORRECTION: Pour décembre avec jour de paie = 2:
   * - Période = 2 décembre 2025 → 1 janvier 2026
   */
  const getPeriodeBudget = useCallback((mois: number, annee: number): PeriodeBudget => {
    ...
    // CORRECTION: La période va du jour de paie du mois ACTUEL au jour avant la paie du mois SUIVANT
    // Ex: Si paie le 2, le budget de décembre = 2 déc au 1 jan
    
    // Date de début = jour de paie du mois actuel
    const debut = new Date(annee, mois, jourPaie);
    ...
    const jourPaieSuivant = getJourPaie(moisSuivant, anneeSuivante);
    
    // Date de fin = jour avant le jour de paie du mois suivant
    const fin = new Date(anneeSuivante, moisSuivant, jourPaieSuivant - 1);
    
    // Si le jour de fin est le 0 (car jourPaieSuivant = 1), prendre le dernier jour du mois actuel
    if (jourPaieSuivant === 1) {
      fin.setTime(new Date(anneeSuivante, moisSuivant, 0).getTime());
    }
