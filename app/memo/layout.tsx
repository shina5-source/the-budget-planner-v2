...
export default function MemoLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="max-w-md mx-auto bg-gradient-to-b from-pink-100 via-purple-50 to-pink-50 min-h-screen relative">
      ...
      <Sidebar isOpen={isSidebarOpen} onClose={() => setIsSidebarOpen(false)} currentPage="memo" onNavigate={handleNavigate} />
      <div className="px-4 pt-4 pb-24">{children}</div>
      <BottomNav currentPage="memo" onNavigate={handleNavigate} onOpenSidebar={() => setIsSidebarOpen(true)} />
    </div>
  );
}