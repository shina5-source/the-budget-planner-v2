...
import { Target, Plus, Edit3, Trash2, X, Check, Lightbulb, ChevronLeft, ChevronRight, RefreshCw, Palmtree, Car, Home, Wallet, Gift, Smartphone, GraduationCap, PartyPopper, Gem, Baby, Heart, ShoppingBasket } from 'lucide-react';
...
export default function ObjectifsPage() {
  ...
  const getRecurrenceLabel = (rec: Objectif['recurrence']) => {
    if (!rec) return '';
    if (rec.frequence === 'mensuel') return `${rec.montant}${parametres.devise}/mois le ${rec.jourDuMois}`;
    if (rec.frequence === 'bimensuel') return `${rec.montant}${parametres.devise} 2x/mois`;
    if (rec.frequence === 'hebdomadaire') return `${rec.montant}${parametres.devise}/sem. (${joursSemaine[rec.jourSemaine || 0]})`;
    return '';
  };
    ...
    <div className="pb-4">
      ...
      {filteredObjectifs.length > 0 ? (
            ...
            return (
              <div key={objectif.id} className={`${couleur.bg} rounded-2xl p-4 shadow-sm border ${couleur.border}`}>
                ...
                <div className="h-3 bg-white/50 rounded-full overflow-hidden mb-2">
                  <div className={`h-full transition-all duration-500 ${isComplete ? 'bg-green-500' : couleur.progress}`} style={{ width: `${Math.min(pourcentage, 100)}%` }} />
                </div>
                ...
                <div className="flex justify-between items-center mb-2">
                  <div className="flex gap-4">
                    ...
                    <div>
                      ...
                      <p className={`text-xs font-medium ${reste > 0 ? couleur.text : 'text-green-600'}`}>{reste > 0 ? reste.toFixed(0) + parametres.devise : '✓'}</p>
                    </div>
                  </div>
                  <div className={`px-2 py-1 rounded-full text-[10px] font-medium ${isComplete ? 'bg-green-500 text-white' : isAlmostComplete ? 'bg-[#D4AF37] text-[#722F37]' : 'bg-white/50 ' + couleur.text}`}>
                ...
                <div className={`pt-2 border-t ${couleur.border} border-opacity-30 space-y-1`}>
                  {objectif.dateEcheance && (
                    <p className={`text-[10px] ${couleur.text} opacity-70`}>📅 Échéance : {formatDate(objectif.dateEcheance)} ({moisRestants} mois)</p>
                  )}
                  {suggestion && !isComplete && (
                    <p className={`text-[10px] ${couleur.text} opacity-70`}>💡 Épargnez {suggestion}{parametres.devise}/mois pour atteindre votre objectif</p>
                  )}
                  {objectif.recurrence && (
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`w-8 h-4 rounded-full transition-colors ${objectif.recurrence.actif ? 'bg-green-500' : 'bg-gray-400'}`}
                      >
                        <div className={`w-3 h-3 rounded-full bg-white shadow transition-transform ${objectif.recurrence.actif ? 'translate-x-4' : 'translate-x-0.5'}`} />
                      </button>
                    </div>
                  )}
                  {isComplete && (
        ...
        <div className={cardStyle + " text-center py-8 mb-4"}>
          ...
          <p className={pageSubtitleStyle + " mb-2"}>{activeFilter === 'tous' ? 'Aucun objectif' : `Aucun objectif ${activeFilter === 'court' ? 'court terme' : 'long terme'}`}</p>
      ...
      <div className={conseilCardStyle}>
        ...
        <div className="space-y-2">
          ...
          {objectifsAvecRecurrence > 0 && (<p className={conseilTextStyle}>🔄 {objectifsAvecRecurrence} objectif(s) avec versement automatique actif</p>)}
          {objectifs.length > 0 && objectifsAvecRecurrence === 0 && (<p className={conseilTextStyle}>💡 Activez les récurrences pour épargner automatiquement</p>)}
        </div>
      </div>
          ...
          <div className="bg-[#8B4557] rounded-2xl p-4 w-full max-w-md border border-[#D4AF37]/40 mb-20 mt-20">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" placeholder="Ex: Voyage Japon, iPhone..." value={formData.nom} onChange={(e) => setFormData({ ...formData, nom: e.target.value })} className={inputStyle} />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className={labelStyle}>Montant cible ({parametres.devise})</label>
                  <input type="number" placeholder="0" value={formData.montantCible} onChange={(e) => setFormData({ ...formData, montantCible: e.target.value })} className={inputStyle} />
                </div>
                <div>
                  <label className={labelStyle}>Déjà épargné ({parametres.devise})</label>
                  <input type="number" placeholder="0" value={formData.montantActuel} onChange={(e) => setFormData({ ...formData, montantActuel: e.target.value })} className={inputStyle} />
                </div>
              </div>

              <div>
                <label className={labelStyle}>Date d'échéance (optionnel)</label>
                <input type="date" value={formData.dateEcheance} onChange={(e) => setFormData({ ...formData, dateEcheance: e.target.value })} className={inputStyle} />
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(['court', 'long'] as const).map((t) => (
                    <button key={t} onClick={() => setFormData({ ...formData, type: t })} className={`py-2 rounded-xl text-xs font-medium border ${formData.type === t ? 'bg-[#D4AF37] border-[#D4AF37] text-[#722F37]' : 'bg-[#722F37]/50 border-[#D4AF37]/50 text-[#D4AF37]'}`}>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(['haute', 'moyenne', 'basse'] as const).map((p) => (
                    <button key={p} onClick={() => setFormData({ ...formData, priorite: p })} className={`py-2 rounded-xl text-xs font-medium border ${formData.priorite === p ? 'bg-[#D4AF37] border-[#D4AF37] text-[#722F37]' : 'bg-[#722F37]/50 border-[#D4AF37]/50 text-[#D4AF37]'}`}>
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {couleursDisponibles.map((couleur) => (
                    <button key={couleur.id} onClick={() => setFormData({ ...formData, couleur: couleur.id })} className={`h-10 rounded-xl border-2 ${formData.couleur === couleur.id ? 'border-[#D4AF37] ring-2 ring-[#D4AF37]' : couleur.border}`}>
              ...
              <div>
                ...
                <div className="grid grid-cols-6 gap-2">
                  {iconesDisponibles.map((icone) => {
                    ...
                    return (
                      <button key={icone.id} onClick={() => setFormData({ ...formData, icone: icone.id })} className={`h-10 rounded-xl flex items-center justify-center border ${formData.icone === icone.id ? 'bg-[#D4AF37] border-[#D4AF37] text-[#722F37]' : 'bg-[#722F37]/50 border-[#D4AF37]/50 text-[#D4AF37]'}`}>
              ...
              <div className={`p-3 rounded-xl border ${formData.recurrenceActif ? 'border-[#D4AF37] bg-[#D4AF37]/10' : 'border-[#D4AF37]/30 bg-[#722F37]/30'}`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => setFormData({ ...formData, recurrenceActif: !formData.recurrenceActif })} className={`w-12 h-6 rounded-full transition-colors ${formData.recurrenceActif ? 'bg-green-500' : 'bg-[#722F37]/50'}`}>
                    <div className={`w-5 h-5 rounded-full bg-white shadow transition-transform ${formData.recurrenceActif ? 'translate-x-6' : 'translate-x-0.5'}`} />
                  </button>
                </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {(['mensuel', 'bimensuel', 'hebdomadaire'] as const).map((f) => (
                          <button key={f} onClick={() => setFormData({ ...formData, recurrenceFrequence: f, recurrenceJour: '1' })} className={`py-1.5 rounded-lg text-[10px] font-medium border ${formData.recurrenceFrequence === f ? 'bg-[#D4AF37] border-[#D4AF37] text-[#722F37]' : 'bg-[#722F37]/50 border-[#D4AF37]/50 text-[#D4AF37]'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input type="number" placeholder="0" value={formData.recurrenceMontant} onChange={(e) => setFormData({ ...formData, recurrenceMontant: e.target.value })} className={inputStyle + " text-xs"} />
                      </div>
                      <div>
                        <label className={smallTextStyle + " block mb-1"}>{formData.recurrenceFrequence === 'hebdomadaire' ? 'Jour semaine' : 'Jour du mois'}</label>
                        <select value={formData.recurrenceJour} onChange={(e) => setFormData({ ...formData, recurrenceJour: e.target.value })} className={inputStyle + " text-xs"}>
          ...
          <div className="bg-[#8B4557] rounded-2xl p-4 w-full max-w-sm border border-[#D4AF37]/40">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="number" placeholder="0" value={versementMontant} onChange={(e) => setVersementMontant(e.target.value)} className={inputStyle} autoFocus />
              </div>
              <div className="flex gap-3">
                <button onClick={() => setShowVersementModal(false)} className="flex-1 py-3 border border-[#D4AF37] text-[#D4AF37] rounded-xl font-medium">Annuler</button>
                <button onClick={handleVersement} className="flex-1 py-3 bg-[#D4AF37] text-[#722F37] rounded-xl font-semibold flex items-center justify-center gap-2">
